...
Jinpeng.mapCarNum.WestFormPanel = Ext.extend(Ext.Panel,{
							            ...
							            'fileselected': function(fb, v){
											...
											form.submit({
												...
												failure : function(form, action) {
													...
													//Ext.getCmp('smallImgBoxOne').getEl().dom.value = 'http://172.31.108.116:8080/a/123.jpg';
													//调用图片框选服务
													...
													window.picInfoIframe.LoadPic(selectImg, function(values) {
														...
														if (rectArr.length < 4) {
															...
															Ext.Msg.alert("系统提示","最多只能框选四处区域,清清楚后重新框选!");
															selectFlag = true;
															return;
														}
													...
													var bodyArea = [];
													bodyArea[bodyArea.length] = 0;
													bodyArea[bodyArea.length] = 0;
													bodyArea[bodyArea.length] = 0;
													bodyArea[bodyArea.length] = 0;
													userArea.BodyArea = bodyArea;
													Ext.Ajax.request({
														method : "POST",
														params : {
															"carImg" : selectImg
														...
														success : function(response, options) {
															var txt = response.responseJSON.data;
															serverImgUrl = txt.imgUrl;
															var json = eval('(' + txt.sjonStr + ')'); 
															Ext.getCmp('carNumStr').setValue(json.VehicleResult[0].RecongnizeResult.PlateResult.Result[0].PlateNumber);
															if (json.VehicleResult[0].RecongnizeResult.PlateResult.Result[0].PlateColor != "") {
																Ext.getCmp('hpys').setValue(json.VehicleResult[0].RecongnizeResult.PlateResult.Result[0].PlateColor + "色");
															}
															var typeValue = json.VehicleResult[0].RecongnizeResult.TypeResult.Result[0].TypeName;
															var typeName = '';
															if (typeValue == '1') {
																typeName = '小型车';
															} else if (typeValue == '2') {
															...
															Ext.getCmp('carCategory').setValue(typeName);
															if (json.VehicleResult[0].RecongnizeResult.ColorResult.Result[0].CarColorName != "") {
																Ext.getCmp('carColor').setValue(json.VehicleResult[0].RecongnizeResult.ColorResult.Result[0].CarColorName + "色");
															}
															Ext.getCmp('carBrand').setValue(json.VehicleResult[0].RecongnizeResult.BrandResult.Result[0].CarTypedName);
															
															if (json.VehicleResult[0].RecongnizeResult.MarkerResult.Result[0].SunRects != null) {
																if ((json.VehicleResult[0].RecongnizeResult.MarkerResult.Result[0].SunRects.length - 1) > 0) {
																	//遮阳板：0或者1
																	Ext.getCmp('sunRects').setValue('1');
																}
															}
															if (json.VehicleResult[0].RecongnizeResult.MarkerResult.Result[0].DropRects != null) {
																if (json.VehicleResult[0].RecongnizeResult.MarkerResult.Result[0].DropRects.length > 0) {
																	//吊坠：0或者1
																	Ext.getCmp('dropRects').setValue("1");
																}
															}
															if (json.VehicleResult[0].RecongnizeResult.MarkerResult.Result[0].PaperRects != null) {
																if (json.VehicleResult[0].RecongnizeResult.MarkerResult.Result[0].PaperRects.length > 0) {
																	//纸巾盒：0或者1
																	Ext.getCmp('paperRects').setValue("1");
																}
															}
															if (json.VehicleResult[0].RecongnizeResult.MarkerResult.Result[0].FlagRects != null) {
																//年检标：多个
																Ext.getCmp('flagRects').setValue(json.VehicleResult[0].RecongnizeResult.MarkerResult.Result[0].FlagRects.length);
															}
															var rects = [];
															var mainObj = {};
															mainObj.BodyArea = json.VehicleResult[0].DetectResult.Result[0].BodyRect;
															mainObj.FaceArea = json.VehicleResult[0].DetectResult.Result[0].FaceRect;
															mainObj.WinArea = json.VehicleResult[0].DetectResult.Result[0].WinRect;
															mainObj.PlateArea = json.VehicleResult[0].RecongnizeResult.PlateResult.Result[0].PlateRect;
															rects[rects.length] = mainObj;
															coordInfo.Rects = rects;//车辆点位信息坐标数组
														},
														failure : function(response, options) {
															Ext.MessageBox.alert("提示信息","图片服务请求失败!"); 
														},
	...
	forwardPage : function(value) {
		...
		if (form.getForm().isValid()) {
			...
			var url = rootpath + '/mapCarNum/mapCarNumResultPage.mvc';
			queryUrl =  "/mapCarNum/mapSearchCar.mvc";
			var title = '以图搜车分析结果';
			var searchCarNum = '';
			...
			params = {
				...
				"pageFlag" : "history",
				"imgUrl" : serverImgUrl,
				"coordInfo" : JSON.stringify( coordInfo ),
				"rects" : JSON.stringify( rects ),
				"userArea" : JSON.stringify( userArea ),
				"similaty" : Ext.getCmp('similaty').getValue(),
				"sunflag" : Ext.getCmp('sunRects').getValue(),
				"dropNum" : Ext.getCmp('dropRects').getValue(),
				"boxNum" : Ext.getCmp('paperRects').getValue(),
				"tagNum" : Ext.getCmp('flagRects').getValue()
			};
