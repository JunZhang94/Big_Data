 ...
 * 目标字节数组中的元素，注意顺序，重复次数
 * 如ab!=ba, aab!=ab
 * @author John
 *
 */
public class RowKeyByteRangeSeqFilter extends FilterBase {
	private int start;
	private int end;
	private byte[] bytes;
	
	public RowKeyByteRangeSeqFilter(){
	}
	
	public RowKeyByteRangeSeqFilter(int start, int end, byte[] bytes){
		this.start=start;
		this.end=end;
		this.bytes=bytes;
	}
	...
	public boolean filterRowKey(byte[] rowkey, int offset, int length) {
		...
		int rowBytesIndex=start;
		
		boolean matchAll = true;
		for (int i = 0; i < bytes.length; i++) {
			boolean match = false;

			for (; rowBytesIndex < rowkey.length && rowBytesIndex < end; rowBytesIndex++) {
