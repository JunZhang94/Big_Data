	...
	viewPortObj = new Ext.Viewport( {
		...
		listeners : {
			afterrender : function() {
				...
				//Ext.getCmp('bigImgBox').getEl().dom.src = config.imgs.bigImg;
				Ext.getDom("bigImgBox").src = config.imgs.bigImg;
				/*
				Ext.getCmp('smallImgBoxOne').getEl().dom.src =  config.imgs.imgOne;
				Ext.getCmp('smallImgBoxTwo').getEl().dom.src =  config.imgs.imgTwo;
				Ext.getCmp('smallImgBoxThree').getEl().dom.src =  config.imgs.imgThree;
				Ext.getCmp('smallImgBoxFour').getEl().dom.src =  config.imgs.imgFour;*/
		...
		Ext.apply(this,{
			...
			listeners : {
				afterrender : function() {
					...
					//定义图片显示
					var iamgeArr = [];
					for (var i = 0; i < imageCondition.length; i++) {
						var tempObj = {};
						tempObj.cls = 'hiddenText';
						tempObj.width = imageCondition[i].width;
						tempObj.height = imageCondition[i].height;
						tempObj.items = imageCondition[i].items[0];
						iamgeArr.push(tempObj);
					}
					for(var i = 0; i < iamgeArr.length; i++){ 
						if (i == 0) {
							Ext.getCmp('imageRegion').add(iamgeArr[i]);
						} else {
							Ext.getCmp('smallimageRegion').add(iamgeArr[i]);
						}
				    }
...
function loadDatas(pbar, cb) {
	tastTime = setInterval(function() {
		Ext.Ajax.request({
			url : rootpath + '/partPicture/checkImageStatus.mvc',
			method : 'POST',
			params : '',
			success : function(resp, opts) {
				var txt = Ext.util.JSON.decode(resp.responseText);
				...
				if (percent >= 1) {
					...
					if (counts > 15) {
						...
						store.proxy = new Ext.data.HttpProxy({url: rootpath + '/partPicture/loadLastDatas.mvc'});
						store.load({
							params : {
								'page.start' : 0,
								'page.limit' : 15
