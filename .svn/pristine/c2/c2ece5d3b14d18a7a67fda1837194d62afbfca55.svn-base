...
(function(){
     ...
     */
     var _XFrameMessaging = function() {
        ...
        window.sendMessage = function(tag,message,domain) {
            var MIF;
                ...
                if (message._fromHost) {
                    ...
                    for (var i = 0, l = mstack.length; i < l; i++) {
                        if (fn = mstack[i]) {
                        	...
                        	result = fn.call(fn.__scope, message,tag,domain);
                            if (fn.__single) {
                                mstack[i] = null;
                            }
                            if (result === false) {
        ...
        window.onhostmessage = function(tag,fn,scope, single) {
        	scope = scope || false,single = single || false;
    ...
    Ext.override( MIFEl ,{
         ...
         * to document.domain).
         */
        sendMessage : function( tag, message,domain) {
            var win, L = location;
            if (this.domWritable() && (win = this.getWindow())) {
                ...
                return win.sendMessage ? win.sendMessage.call(null, tag,message,  domain) : undefined;
            }
            return;
        },
        ...
        postMessage : function(message, origin, target ){
            ...
            if(w && !this.disableMessaging){
                ...
                message = Ext.isObject(message) || Ext.isArray(message) ? message : message;
