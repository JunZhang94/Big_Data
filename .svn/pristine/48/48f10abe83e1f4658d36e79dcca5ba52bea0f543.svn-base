...
//历史过车查询功能JS
Ext.ns("Jinpeng.OneCarManyNumQuery");
...
//查询表单
Jinpeng.OneCarManyNumQuery.NorthFormPanel = Ext.extend(Ext.Panel,{
	id : 'categoryPanel',
...
//							anchor : '94%'
							...
							listeners  : {   
				            	'focus':function(field){  
								    ...
								    {   
								        ...
								        this.dateFmt    = 'yyyy-MM-dd HH:mm:ss';  //  格式化时间   
								        this.autoPickDate = true; this.isShowWeek = false;                  //  默认不显示周   
								        this.alwaysUsestartTime = false;           //  默认使用初始时间   
								    };  
				                    WdatePicker(defaultDateTimeParams);   
				                    field.blur();
				             	}   
					...
					items : [{
						...
						items : [{
							...
							items : [{
								...
								//handler : this.categoryQueryMethod
							}, {
								flex : 31,
								xtype : 'button',
								cls : 'buttunreset',
		...
		Jinpeng.OneCarManyNumQuery.NorthFormPanel.superclass.initComponent.apply(this);
	},
	//第二种通过纯js方式实现跳转
	...
	forwardPage : function(value) {
		var form = Ext.getCmp('categoryQueryForm');
		if (form.getForm().isValid()) {
			...
			var url = rootpath + '/carType/generalQueryResultPage.mvc';
			queryUrl =  "/oneCarManyNum/queryOneCarManyNum.mvc";
			var title = '一车多牌分析结果';
			params = {
				...
				"dropNum":Ext.getCmp('dropNum').getValue() == false ? '0' : '1',
				"boxNum":Ext.getCmp('boxNum').getValue() == false ? '0' : '1',
//				"sunflag":Ext.getCmp('sunflag').getValue() == false ? '0' : '1',
				"tagNum":Ext.getCmp('tagNum').getValue(),
				"operate":Ext.getCmp('operate').getValue(),
				"flag" : "query",
				"groupFlag" : groupFlag,
				"pageFlag" : "category",
				"modelExportType" : "oneCarManyNum"
			};
			//定义查询条件
			conditions = [{
				items : [ {
				...
				items :[{
					...
					id : 'tagNum',
					fieldLabel : '年检标',
					value:Ext.getCmp('operate').getValue()+Ext.getCmp('tagNum').getRawValue(),
					anchor : '94%'
				 }]
			},{
				items :[{
					...
					id : 'dropNum',
					fieldLabel : '吊坠',
					value:Ext.getCmp('dropNum').getValue()==true?"有":"无",
					anchor : '94%'
				 }]
			},{
				items :[{	
					...
					id : 'boxNum',
					fieldLabel : '纸巾盒',
					value:Ext.getCmp('boxNum').getValue()==true?"有":"无",
					anchor : '94%'
				 }]
			},{
				items : [ {
				...
				items : [ {
					...
					id : 'startTime',
					name : 'startTime',
					value : Ext.getCmp('startTime').getValue(),
					anchor : '96%'
				} ]
			},{
				items : [ {
					...
					id : 'endTime',
					name : 'endTime',
					value : Ext.getCmp('endTime').getValue(),
					anchor : '96%'
				} ]
			},{
				items : [{
	...
	resetMethod : function() {
		clearSelect();
		Ext.getCmp('categoryQueryForm').getForm().reset();
...
Ext.reg('northFormPanel', Jinpeng.OneCarManyNumQuery.NorthFormPanel);