...
Ext.onReady(function() {
	...
	viewPort =  new Ext.Viewport({
		...
		items : [{
			...
			region : 'center',
			border : false,
			xtype : 'statisticsDataPanel'
		}]
	});
});

Jinpeng.bayonet.StatisticsDataPanel = Ext.extend(Ext.Panel,{
	frame:true,
    collapsible:true,
    layout:'fit',
    border : true,
	initComponent : function() {
		...
		Jinpeng.bayonet.StatisticsDataPanel.superclass.initComponent.apply(this);
	}
});
...
Jinpeng.bayonet.DeptStatisticsGridPanel = Ext.extend(Ext.grid.GridPanel, {
	...
	initComponent : function() {
		...
		deptStatisticsStore = new Ext.data.GroupingStore({
			url : rootpath+ "/dc/deptStatisticsGroupping.mvc",
			...
			colModel  : new Ext.grid.ColumnModel({
				columns : [sm, {
		            ...
		            summaryRenderer: function(v, params, data){
		        		var counts = data.data.COUNS;
				    	var nonHphmCouns = data.data.NON_HPHM_COUNS;
				    	var hphmCouns = data.data.HPHM_COUNS;
	                    return '接收总数 ('+counts+')，已识别总数 (<span style="color:green;">'+nonHphmCouns+'</span>)，未识别总数 (<span style="color:red;">' + hphmCouns +'</span>)'; 
	                }
				},{
		            header: '接收数量',
		            dataIndex: 'COUNS',
...
Jinpeng.bayonet.trafficStatisticsGridPanel = Ext.extend(Ext.grid.GridPanel, {
	...
	initComponent : function() {
		...
		trafficStatisticsStore = new Ext.data.GroupingStore({
			url : rootpath+ "/dc/trafficStatisticsGroupping.mvc",
			sortInfo: {field: 'STATISTICAL_TIME', direction: 'ASC'},
			groupField: 'KKMC',
			reader:reader
		});
		...
		Ext.apply(this, {
			...
			colModel  : new Ext.grid.ColumnModel({
				columns : [sm, {
					...
					width : 300,
		            summaryRenderer: function(v, params, data){
		        		var counts = parseInt(data.data.COUNS);
				    	var nonHphmCouns = parseInt(data.data.NON_HPHM_COUNS);
				    	var hphmCouns = parseInt(data.data.HPHM_COUNS);
	                    return '接收总数 ('+counts+')，已识别总数 (<span style="color:green;">'+nonHphmCouns+'</span>)，未识别总数 (<span style="color:red;">' + hphmCouns +'</span>)'; 
	                }
				},{
		            header: '接收数量',
...
Ext.reg('statisticsDataPanel', Jinpeng.bayonet.StatisticsDataPanel);
