...
Ext.onReady(function() {
	...
	//默认加载查询功能
	Ext.getCmp('historyPanel').historyQueryMethod();
});
...
Jinpeng.historyQuery.NorthFormPanel = Ext.extend(Ext.Panel,{
				...
				items : [ {
					...
					items : [{
						...
						items : [ {
							...
							value :_panel.getdate4H(new Date(),1),
							anchor : '94%',
		...
		historyQueryMethod : function() {
			...
			if (form.getForm().isValid()) {
				...
				if (!flag) {
					if (Ext.getCmp('directions').getValue()) {
						if (Ext.getCmp('directions').getValue().split(',').length > 1) {
							var seconds = (endDate1.getTime() - startDate1.getTime())/1000;
							if (seconds > (24 * 60 * 60)) {
								if (showFlag == 'grid') {
									Ext.getCmp('timeDesc').setVisible(true);
 ...
 '<ul>',
     '<tpl for=".">',
         '<li class="phone">',
             ...
             '<strong>车牌号码：<a href="#" onclick=\'showCarPlace("{hphm}")\'>{hphm}</a></strong>',
...
//DataView区域
Jinpeng.historyQuery.DataView = Ext.extend(Ext.DataView,{
	id: 'phones',
	...
	initComponent : function() {
		...
		Jinpeng.historyQuery.DataView.superclass.initComponent.apply(this);
	}
});

//panel区域
Jinpeng.historyQuery.DataPanel = Ext.extend(Ext.Panel,{
	frame:true,
    //autoHeight:true,
    height : Ext.getBody().getHeight() - 90,
    collapsible:true,
	...
	initComponent : function() {
		...
		Jinpeng.historyQuery.DataPanel.superclass.initComponent.apply(this);
	},
	//导出Excel格式数据方法 
	importExcelData : function() {
...
Ext.reg('historyDataView', Jinpeng.historyQuery.DataView);
Ext.reg('dataPanel', Jinpeng.historyQuery.DataPanel);
