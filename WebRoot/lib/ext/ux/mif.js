  ...
  var frameEvents = ['documentloaded',
    ...
    Ext.ux.ManagedIFrame.Element = Ext.extend(Ext.Element, {
            ...
            constructor : function(element, forceNew, doc ){
               ...
               this.dom.ownerCt = this;
                ...
                if(!this._observable){
                 ...
                 if(!Ext.isIE6)//修复在ie6中刷新页面奇怪的内存泄漏
            ...
            destructor   :  function () {
              ...
              //修复在ie6中刷新页面奇怪的内存泄漏
                if(Ext.isIE){
                	//下面一句ie8下，关闭最后一个tab会一起ie崩溃，注释掉
//                	this.dom.contentWindow.document.write('');//清空iframe的内容 
    				this.dom.contentWindow.close();//避免iframe内存泄漏
                }
            },
...
//	                        this._frameAction  
            ...
            checkDOM : function( win) {
                ...
                (function() { // DOM polling for IE and others
                    ...
                    if (d.location.href && !domReady && (++n < max)) {
                        setTimeout(arguments.callee.apply(this), 2); // try again
                        return;
                    }
            ...
            */
	         ...
	         * to document.domain).
	         */
	        sendMessage : function(tag, message, origin) {
...
//                frames[frame.id] = frames[frame.name] = {ref : frame };
            ...
            toArray : function() {
            	var arr = [];
            	Ext.iterate(frames, function(k, v) {arr.push({name: k, ref: v.ref});},this);
            	return arr;
            }, 
            /** @private */
            ...
            destroy : function() {
                ...
                if(Ext.isIE){//修复ie6,7内存泄漏
                	Ext.iterate(frames,function(prop,frame){
                		if(frame.ref.getWindow()._frameOnUnload){
                			frame.ref.getWindow()._frameOnUnload();
                		}
                		if(frame.ref)
                			frame.ref.remove();
                	});
        ...
        Ext.EventManager.on(window, 'unload', implementation.destroy, implementation);
