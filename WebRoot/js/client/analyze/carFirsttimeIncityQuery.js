 ...
 * 初次入城
 */
Ext.ns("Jinpeng.carFirsttimeIncityQuery");
 ...
 * north区域表单部份
 */
Jinpeng.carFirsttimeIncityQuery.FirstTimeIncityFormPanel = Ext.extend(Ext.Panel,{
	id:'formId',
	initComponent : function() {
		...
		Jinpeng.carFirsttimeIncityQuery.FirstTimeIncityFormPanel.superclass.initComponent.apply(this);
	},
	/* 响应分析按钮 */
	alarmSearch: function() {
		...
		if (form.getForm().isValid()) {
			...
			var url = rootpath + '/firstTimeInCity/firstTimeInCityResult.mvc';
			var title = '初次入城查询结果';
...
function getFormParamsForSend(){
	...
	var baseparams = {
		...
		"kkbh" : Ext.getCmp('directions').getValue()
	};
	return baseparams;
}
//构造导出传参结构
function getExportParms(){
	/** 将参数传入后台 */
		...
		param[param.length] ="kkbh=" + Ext.getCmp('directions').getValue()
...
function getFormForCondictions(){
	...
	var conditions = [{
		items :[{
			...
			name : 'carNumStr',
			id : 'carNumStr',
			value : Ext.getCmp('hphm').getValue(),
			anchor : '96%'
		}]
	}, {
		items : [ {
			...
			id : 'exceptHphm',
			name : 'exceptHphm',
			value : Ext.getCmp('exceptHphm').getValue(),
			anchor : '96%'
		} ]
	}, {
		items : [ {
		...
		items : [ {
			...
			id : 'backTimeLen',
			name : 'backTimeLen',
			value : Ext.getCmp('backTimeLen').getValue(),
			anchor : '96%'
		} ]
	},{
		items : [ {
...
Ext.reg('firstTimeIncityFormPanel', Jinpeng.carFirsttimeIncityQuery.FirstTimeIncityFormPanel);
Ext.reg('firstTimeIncityDataPanel', Jinpeng.carFirsttimeIncityQuery.FirstTimeIncityPanel);
