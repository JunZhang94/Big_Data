Ext.ns('Jinpeng.blackFile');

Jinpeng.blackFile.UploadWindow = Ext.extend(Jinpeng.widget.GeneralWindow, {
	/**
	 * 允许上传的文件类型
	 */
	allowFileType : [ ".xls" ],
	...
	initComponent : function() {
					...
					if (form.isValid()) {
						...
						form.submit({
							...
							failure : function(form, action) {
								...
								var msg = "";
								if (carNumStr) {
									msg = '导入数据成功,您一共导入了' + carNumStr + '条记录。';
								} else {
									msg = '导入数据失败！';
								}
								win.msg = msg;
		...
		Jinpeng.blackFile.UploadWindow.superclass.initComponent.apply(this, arguments);
	},
	afterRender : function() {
		Jinpeng.blackFile.UploadWindow.superclass.afterRender.apply(this, arguments);
		if (this.templateURL) {
