...
Jinpeng.controlRevokeVerify.DafaultParam = {
	'page.start' : 0,
 ...
 * @class 布控查询面板
 */
Jinpeng.controlRevokeVerify.SearchFormPanel = Ext.extend(Ext.Panel, {
	defaults : {
		margins : '0 0 0 4'
	},
	initComponent : function() {
		...
		Ext.apply(this, {
			items : [ {
				...
				items : [ {
					...
					value : Jinpeng.controlRevokeVerify.DafaultParam['beginDate']
				}, {
					xtype : 'datetimefield',
					editable : false,
					fieldLabel : '至',
					...
					width : 145,
					value : Jinpeng.controlRevokeVerify.DafaultParam['endDate']
				}, {
					xtype : 'button',
					text : '&nbsp;&nbsp;&nbsp;查询&nbsp;&nbsp;&nbsp;',
		...
		Jinpeng.controlRevokeVerify.SearchFormPanel.superclass.initComponent.apply(this);
		// 初始化页面参数
 ...
 * @class 布控信息列表
 */
Jinpeng.controlRevokeVerify.QueryContorlCarGridPanel = Ext.extend(Ext.grid.GridPanel, {
	...
	initComponent : function() {
		var store = new Ext.data.JsonStore({
			url : rootpath + '/controlManager/queryControl.mvc?queryType=revokeVerify',
			root : 'result',
		...
		Ext.apply(this, {
			...
			tbar : {
				...
				items : [ {
					...
					text : '&nbsp;&nbsp;&nbsp;审核&nbsp;&nbsp;&nbsp;',
					id : 'verifyBtn',
					handler : this.onControlRevokeVerifyHandler,
					scope : this
				} ]
			},
		...
		Jinpeng.controlRevokeVerify.QueryContorlCarGridPanel.superclass.initComponent.apply(this);
	},
	afterRender : function() {
		Jinpeng.controlRevokeVerify.QueryContorlCarGridPanel.superclass.afterRender.apply(this);
		this.store.baseParams = Jinpeng.controlRevokeVerify.DafaultParam;
		this.store.load(); // 加载数据
	},
	onControlRevokeVerifyHandler : function() {
		...
		if (this.isNoRecordSelected()) {
			...
			if (ids.length == 1) {
				...
				var revokeVerifyWindow = new Jinpeng.controlRevokeVerify.ControlRevokeVerifyWindow({
					disableBtn : false
				});
				revokeVerifyWindow.init({
					ids : ids.join(","),
	...
	isNoRecordSelected : function() {
		var records = Ext.getCmp('ControlInfoGridPanel').getSelectionModel().getSelectoions();
		return records.length == 0 ? true : false;
	}

});
Jinpeng.controlRevokeVerify.ControlRevokeVerifyWindow = Ext.extend(Jinpeng.widget.GeneralWindow, {
	width : 400,
	height : 200,
	disableBtn : true,
	cls : "blue-button-ct",
	constructor : function(config) {
		...
		this.items = this.fp;
		Jinpeng.controlRevokeVerify.ControlRevokeVerifyWindow.superclass.constructor.apply(this, arguments);
	},
...
Ext.reg('searchformpanel', Jinpeng.controlRevokeVerify.SearchFormPanel);
Ext.reg('querycontrolcargrid', Jinpeng.controlRevokeVerify.QueryContorlCarGridPanel);
