...
//查询表单Jinpeng.realTimeQuery.NorthFormPanel = Ext.extend(Ext.Panel,{	id : 'formPanel',
	initComponent : function() {		var _panel = this;		Ext.apply(this,{			items : [ {				// form表单				xtype : 'form',				id : 'realTimeQueryForm',				border : false,				frame : true,				cls : 'blue-button-ct',				layout : 'table',				defaults : {					layout : 'form',					width : 700				},				bodyStyle : 'background-color: #F7F7F7;border: 1px solid #DDDDDD;margin-left:-10px;',				items : [{					items : [ {						xtype : 'compositefield',						items : [ {							flex : 0.5,							fieldLabel : '卡点',							xtype : 'tooltiptextfield',							name : 'kkmcs',							id : 'kkmcs',							width : 445,							emptyText : '请选择卡点'						}, {							flex : 0.5,							owner : this,							labelAlign : 'right',							xtype : 'button',							text : '选择卡口',							id:'choosekkBtn',							handler : function(){								kwin.show();							}						} ]					} ]				},{
		...
		Jinpeng.realTimeQuery.NorthFormPanel.superclass.initComponent.apply(this);
	},	//查询	historyQueryMethod : function() {		var form = Ext.getCmp('realTimeQueryForm');
		if (form.getForm().isValid()) {
			var grid = Ext.getCmp('realTimeGridPanel');
...
//中心右区域数据
Jinpeng.realTimeQuery.gridCenterDataPanel = Ext.extend(Ext.grid.GridPanel,{
	id : 'realTimeGridPanel',
	border : false,
	frame : true,	height :  Ext.getBody().getHeight() - 48,
	pageSize : Jinpeng.PageSize,
	initComponent : function() {
		Ext.apply(this,{
			store : realTimeQueryStore,
			cm : new Ext.grid.ColumnModel({
				...
				rowdblclick : function(grid, rowIndex, e ) {
					...
					var win = new Jinpeng.realTimeQuery.RealTimeDetailWindow();
					win.recode = data;
					//将ID设置到弹出框win的属性中，用于win完成渲染后加载数据。
					win.loadId = data.get("xxbh");
					win.tx1 = data.get("tx1");
					win.show();
				},				rowclick : function(grid, rowIndex, e) {					var data = grid.store.getAt(rowIndex);					var kkbh = data.get("kkbh");					var kkmc = data.get("kkmc");					if (kkmc != null && kkmc != '') {						window.realTimeIframe.GIS_Clear();//清除选择						window.realTimeIframe.GIS_ZoomTo(kkbh);					}				}
			}
		});		Jinpeng.realTimeQuery.gridCenterDataPanel.superclass.initComponent.apply(this);
	}});//弹出窗口历史过车详细信息Jinpeng.realTimeQuery.RealTimeDetailWindow = Ext.extend(Jinpeng.widget.GeneralWindow,{
	width : 940,
	height : 550,
	closeAction : "close",
	title : '详细信息',
	...
	getDetailRecordId : function(id, upOrDown) {
		...
		if(id!=null){
			...
			for ( var i = 0; i < realTimeQueryStore.getCount(); i++) {
				if (this.loadId == realTimeQueryStore.getAt(i).get('xxbh')) {
					k = i;
					break;
				}
		    }
		...
		if (upOrDown == false && k > 0) {
			//测试用的
			//nextId = (rownum-k)+'|'+realTimeQueryStore.getAt(k-1).get('xxbh');
			//信息编号唯一的情况下使用
			nextId = (k-1)+'|'+realTimeQueryStore.getAt(k-1).get('xxbh');
		}
		if (upOrDown == true
				&& k < realTimeQueryStore.getCount() - 1) {			
			//nextId = (k+rownum+1)+'|'+realTimeQueryStore.getAt(k+1).get('xxbh');
			nextId = (k+1)+'|'+realTimeQueryStore.getAt(k+1).get('xxbh');	
		}
		if(nextId) {
			this.loadId = nextId;
	...
	synchronUpOrDown : function(id) {
		var count = realTimeQueryStore.getCount();
		var index = 0;
		for ( var i = 0; i < count; i++) {
			if (id == realTimeQueryStore.getAt(i).get('xxbh')) {
				index = i;
				break;
			}
		}
