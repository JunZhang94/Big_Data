				...
				<div class="loginBox">
					<form action="${pageContext.request.contextPath}/user/to/login.mvc" method="post" onkeyup="enterSubmit(event)" style="margin-top: -75px;">
						<div style="height: 42px; clear: both; margin-top: 14px; margin-bottom: 17px;">
							<input class="us" type="text" style="width: 252px; height: 35px; padding-left: 9.99px; margin-right: 39px; margin-left: 13.99px;"
							 name="userName" placeholder="请输入账号" id="userId" onkeydown="toNext(this.form)" />
						</div>
						<div style="height: 42px; padding-top: 1px; padding-bottom: 0px; clear: both; margin-top: 14px; margin-bottom: 17px;">
							<input class="pw" type="password" style="height: 34px; padding-bottom: 1.99px; width: 252px; margin-right: 39px;" name="password"
							 placeholder="请输入您的密码…" />
						</div>
						<input type="submit" class="btn1 login" value="" />
						<!-- 检测身份证书 -->
						<input value="身份认证" class="sfrz" type="button" onclick="function getori(){window.location = '/Big_Data/random';} getori(); ">
						<jsp:include page="inspect.jsp"></jsp:include>
						<!-- 结束 -->
